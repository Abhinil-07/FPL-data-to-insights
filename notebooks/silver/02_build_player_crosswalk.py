...
# COMMAND ----------
# 2. Historical Players Snapshot
hist_players = spark.read.table(f"{db_bronze}.archive_players_raw").select(
...
# COMMAND ----------
# 3. Match Historical Players to Live player_key
# Match Strategy 1: Match by durable FPL code where code exists in both live and historical data
code_matched = hist_players.join(
    live_players.select("player_key", "norm_full_name"),
    hist_players.hist_code == live_players.player_key,
    "inner"
).select(
    F.col("player_key"),
    F.col("season"),
...
# Match Strategy 2: Match by normalized full name for older seasons where code was missing
name_matched = hist_players.join(
    live_players.select("player_key", "norm_full_name"),
    hist_players.hist_norm_full_name == live_players.norm_full_name,
    "inner"
).select(
    F.col("player_key"),
    F.col("season"),
    ...
    F.col("hist_first_name").alias("first_name"),
    F.col("hist_second_name").alias("second_name"),
    F.lit("matched_by_name").alias("crosswalk_status")
)
...
crosswalk_union = live_entries.unionByName(code_matched).unionByName(name_matched).distinct()
...
display(crosswalk_union.limit(20))
