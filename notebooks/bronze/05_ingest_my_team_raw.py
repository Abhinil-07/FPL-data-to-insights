...
# Ingest ONLY `fpl_bronze.my_team_raw` from entry/{TEAM_ID}/ endpoint (Optional)
...
if team_id:
    ...
    if team_data:
        ...
        my_team_df.write.mode("overwrite").format("delta").saveAsTable(target_table)
        print(f"✅ Successfully written squad data for Team ID {team_id} to Delta table: {target_table}")
        display(my_team_df)
    else:
